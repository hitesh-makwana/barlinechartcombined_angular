...
module.exports = function(Chart) {
	...
	Chart.Scale = Chart.Element.extend({
		...
		//custom function To group lable and format into month year format
		//formatLblToMonthFn: 
        /*group label by month, year
        @label: Current Label
        @Index: Current Label Index
        @labels: Labels Array
        @monthLngNames:Month Names
        */
		formatLblToMonthFn:function (label, index, labels,monthLngNames) {
                //Check current label month match with previous label
	            var currLblYear = labels[index].substr(6,4);
	            var currLblMonth=labels[index].substr(3,2);
	            var formatedLbl = monthLngNames[parseInt(currLblMonth)-1]+' '+currLblYear;
	            if(index==0 || (index>0 &&  currLblMonth!= labels[index-1].substr(3,2))){
	                return '                                      '+formatedLbl;
	            }else{
	                return null;
	            }
		...
		draw: function(chartArea) {
			...
			helpers.each(me.ticks, function(label, index) {
				...
				if(me.options.groupByLabel){
					label = me.formatLblToMonthFn(label, index,me.ticks,me.options.monthLabels);					
				}
...
module.exports = function(Chart) {
	...
	Chart.defaults.global.tooltips = {
		...
		borderWidth: 0,
        paddingAdjustment:0,
		toolTipRiskMonitorTemplate:false,
		callbacks: {
	...
	function getBaseModel(tooltipOpts) {
		...
		return {
			...
			bodyFontColor2: tooltipOpts.bodyFontColor2,
			_bodyFontFamily: getValueOrDefault(tooltipOpts.bodyFontFamily, globalDefaults.defaultFontFamily),
			_bodyFontFamily2: getValueOrDefault(tooltipOpts.bodyFontFamily2, globalDefaults.bodyFontFamily),
			_bodyFontStyle: getValueOrDefault(tooltipOpts.bodyFontStyle, globalDefaults.defaultFontStyle),
			_bodyFontStyle2: getValueOrDefault(tooltipOpts.bodyFontStyle2, tooltipOpts.bodyFontStyle),
			_bodyAlign: tooltipOpts.bodyAlign,
			bodyFontSize: getValueOrDefault(tooltipOpts.bodyFontSize, globalDefaults.defaultFontSize),
			bodyFontSize2: getValueOrDefault(tooltipOpts.bodyFontSize2, tooltipOpts.bodyFontSize),
			bodySpacing: tooltipOpts.bodySpacing,
			_paddingAdjustment: tooltipOpts.paddingAdjustment,
	...
	Chart.Tooltip = Chart.Element.extend({
		...
		getBody: function(tooltipItems, data) {
			...
			helpers.each(tooltipItems, function(tooltipItem) {
				...
				if(me._options.toolTipRiskMonitorTemplate)				
					bodyItem.lines =me.breakItemsLines(bodyItem.lines);				
		...
		// Args are: (itemlines)
		breakItemsLines:function(itemlines){
			var newArr =[];
			for(var i=0;i<itemlines.length;i++){
					var itemArr = itemlines[i].split(":");
					if(itemArr.length>1){
						
						if( itemArr[0] == 'EXPOSURE TREND'){
							newArr.push(itemArr[1]+'%');	
						}
						if( itemArr[0] == 'EXPOSURE'){
							newArr.push(parseFloat(itemArr[1]).toLocaleString());	
						}
						newArr.push(itemArr[0]);
					}
				}
		...
		drawTitle: function(pt, vm, ctx, opacity) {
			...
			var paddingAdjustment = vm._paddingAdjustment;
			if (title.length) {
				...
				for (i = 0, len = title.length; i < len; ++i) {
					ctx.fillText(title[i], pt.x+paddingAdjustment, pt.y);
		...
		drawBody: function(pt, vm, ctx, opacity) {
			...
			var bodyFontSize2 = vm.bodyFontSize2;
			...
			var xLinePadding = 0;
			var paddingAdjustment = vm._paddingAdjustment;
			var fillLineOfText = function(line,fontColor) {
				ctx.fillStyle = fontColor;

				if(line=='EXPOSURE' || line=='EXPOSURE TREND'){
					ctx.font=helpers.fontString(bodyFontSize2, vm._bodyFontStyle2, vm._bodyFontFamily2);	
				}else{
					ctx.font = helpers.fontString(bodyFontSize, vm._bodyFontStyle, vm._bodyFontFamily);
				}
				
				ctx.fillText(line, pt.x + xLinePadding+paddingAdjustment, pt.y);
			...
			helpers.each(body, function(bodyItem, i) {
				...
				helpers.each(bodyItem.lines, function(line) {
					...
					if(vm.labelColors[i].backgroundColor == '#CCCCCC')//TBD
						vm.labelColors[i].backgroundColor =vm.bodyFontColor;
					
					fillLineOfText(line,vm.labelColors[i].backgroundColor);
				});
